...
function StatusCard({ title, status, color }) {
  ...
  return (
    <div className={`${colors.bg} border ${colors.border} p-6 rounded-lg shadow-sm`}>
...
const [alreadyApplied, setAlreadyApplied] = useState(false);
  ...
  try {
    const payload = {
      ...
      isSportsMember: formData.isSportsMember,
      examStatus: formData.examStatus,     // 👈 ADD
  examRemark: formData.examRemark       // 👈 ADD
    };
...
>
  ...
  <option value="regular">Regular (No Backlog)</option>
  <option value="backlog">Backlog</option>
  <option value="sem_break">Semester Break</option>
  <option value="year_break">Year Break</option>
</SelectField>
{formData.examStatus && formData.examStatus !== "regular" && (
  <div>
    <label className="block text-sm font-medium text-gray-700 mb-2">
      Exam Remark (optional)
    </label>
    <textarea
      name="examRemark"
      value={formData.examRemark}
      onChange={handleChange}
      placeholder="Example: Backlog in Maths / Year break approved"
      rows="3"
    ...
    />
          ...
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
            <p className="text-sm text-blue-900">
              <strong>Note:</strong> Your application will be reviewed by Library,
              Accounts, and Hostel departments.
            </p>
          </div>
...
function ViewStatus() {
 const rollNumber = localStorage.getItem("rollNumber")?.trim();
  ...
  <div className="max-w-3xl">
    ...
    <DepartmentStatus
      department="HOD"
      status={statusData.hod}
      statusType={
        statusData.hod === "approved" ? "success" : "warning"
      }
      message={
        statusData.hod === "approved"
          ? "Final Approved"
          : "Pending review"
      }
    />
...
function DepartmentStatus({ department, status, statusType, date, message }) {
  const statusStyles = {
    ...
    <div className={`${styles.bg} border ${styles.border} rounded-lg p-5`}>
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-2">Last Updated: {date}</p>
        </div>
        <span className={`${styles.badge} text-white px-3 py-1 rounded-full text-sm font-medium`}>
          {status}
        </span>
      </div>
    </div>
    ...
    <div className="flex items-start">
      <div className="flex flex-col items-center mr-4">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
          completed ? "bg-green-600 border-green-600" : "bg-white border-gray-300"
        }`}>
          {completed && <span className="text-white text-sm">✓</span>}
        </div>
        {!isLast && (
          <div className={`w-0.5 h-12 ${completed ? "bg-green-600" : "bg-gray-300"}`} />
        )}
      </div>
      <div className="flex-1 pb-6">
        <h3 className={`font-semibold ${completed ? "text-gray-900" : "text-gray-500"}`}>
          {label}
        </h3>
